...
SELECT 'ETH' AS symbol, x.* FROM (
  SELECT
    ...
    hash
  FROM evms.transactions b
  JOIN getCEXMapping a ON address = "from"
  LEFT JOIN prices.usd q
    ON DATE_TRUNC('minute', block_time) = minute
    AND symbol = 'WETH'
    AND q.blockchain = 'ethereum'
  WHERE block_time >= TIMESTAMP '2024-03-31'
    AND block_time < TIMESTAMP '2024-04-01'
    AND data = 0x
    AND b.blockchain IN ('ethereum', 'optimism', 'arbitrum', 'base')
    AND value > 0
) x
LEFT JOIN getCEXMapping ON recipient_address = address
WHERE name IS NULL
...
SELECT
  ...
  sender_address,
  CASE
    WHEN LOWER(x.name) LIKE '%binance%' THEN 'Binance'
...
FROM (
  SELECT
    ...
    symbol,
    a.blockchain,
    a."from" AS sender_address,
    name,
    c."to" AS recipient_address,
    c.value / 1e6 AS amount_funded_usd,
    hash,
    data
  FROM evms.transactions a
  JOIN mapping b ON "to" = contract_address AND a.blockchain = b.blockchain
  JOIN evms.erc20_transfers c
    ON a.block_number = c.evt_block_number
    AND a.hash = c.evt_tx_hash
  JOIN getCEXMapping d ON a."from" = d.address
  WHERE block_time >= TIMESTAMP '2024-03-31'
    AND block_time < TIMESTAMP '2024-04-01'
) x
LEFT JOIN getCEXMapping tt ON recipient_address = address
